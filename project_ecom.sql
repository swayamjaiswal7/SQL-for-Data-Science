...
select * from orders;
-------------------------
-- top3 outlets by cuisine type without limit and top function
with cte as (
...
-- Find daily new customer count from the launch date (Everyday count of customer
-- Acquired)
with cte as (
Select Customer_code, cast(Min(placed_at) AS date) as first_order_date 
from orders
group by Customer_code)
Select first_order_date ,count(*) as new_cust
from cte
group by first_order_date
order by first_order_date
;
-- Set strict and safe SQL mode for this session only

SET SESSION sql_mode = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
select @@SESSION.sql_mode;

-- 3. Count of All user who were acquired in Jan 2025 
-- and only one order in
-- Jan and not placed any other order
...
-- 4 list all customers with no order in last 7 days but were acquired one month ago
...
and first_order_date < date_sub(curdate() ,interval 7 day) and orders.Promo_code_Name is not null
;

-- 5) Growth team is plannning to create a trigger that will customers after their
-- event third order is a personalized commmunication and they have asked you to 
-- create a query for this
with cte as (
SELECT *, row_number() over 
(partition by customer_code order by placed_at ) as Order_number
from orders)
Select * from cte where order_number%3=0 and cast(placed_at as date) = cast('2025-03-31' as date);

-- 6) List customers who placed more than 1 orders and all their orders on a promo only
select customer_code , Count(*) as No_Orders,Count(Promo_code_name) as promo_orders
from orders
group by customer_code
having count(*)>1 and Count(*) = count(promo_code_name);

-- 7) What % of Customers were organically Acquired In Jan 2025 ( Placed their 1st order without 
-- Promo code
